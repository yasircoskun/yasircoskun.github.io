  ...
  while ((m = regex.exec(source_code)) !== null) {
      ...
      m.forEach((match, groupIndex) => {
        if(groupIndex == 0){
          ...
          source_code = source_code.replaceAll(match, match.split(end_of_statement)[0]+"33"+end_of_statement);
  ...
  while ((m = regex.exec(source_code)) !== null) {
      ...
      m.forEach((match, groupIndex) => {
        if(groupIndex == 0){
          ...
          console.log(`Found match, group ${groupIndex}: ${match}`);
          console.log(match.split(end_of_statement)[0]+"2");
        }
...
if(window.location !== window.parent.location){
  loadSource("https://cdnjs.cloudflare.com/ajax/libs/bootstrap/5.0.2/css/bootstrap.min.css",'text/css')
  loadSource("https://cdn.jsdelivr.net/gh/dealfonso/pdfjs-viewer@1.0.0/pdfjs-viewer.min.css",'text/css')
  // loadSource("https://cdn.jsdelivr.net/npm/bootstrap@5.1.3/dist/css/bootstrap.min.css",'text/css')
  document.addEventListener("DOMContentLoaded", function() {
    document.body.style.filter = "contrast(1) grayscale(1) invert(1) brightness(1.5)"
